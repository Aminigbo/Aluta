...
import {SentimentVeryDissatisfied,CheckCircle,FileCopyOutlined} from '@material-ui/icons';
...
function Home({ appState, logout,walletAdd,getOneOnOne,oneXoneResultAdd}) {
  ...
  React.useEffect((compState) => {  
    ...
    setStates({ ...compState, loader: true,copy:false})
...
// RENDER ALL CHALLENGES
 ...
 async function createChallenge(){ 
    ...
    if (choice === "Your choice" || amount.length < 1 || choice.length < 1) {
      ...
      const radm6 = radm5.shuffle().substring(4, 9).shuffle();
      ...
      if (parseInt(amount) > parseInt(state.wallet)) {
        ...
        .then(response => { 
          ...
          if (response.body.length < 1) {
            ...
            .from('challenge')
            .insert([
              { host: id, match: hostData.matche.id,amount,data:hostData,status:1, isActive:true, code:challengeCode},
            ])
              .then(created => {
                const meta =  { host: id, match: hostData.matche.id,amount,data:hostData,status:1, isActive:true, code:challengeCode}
                new_supabase.from("feeds").insert([{ type: "NEW CRAETED CHALLANGE", from: 'ADMIN', to: "ALL", meta: meta }]).then(res2 => {
                  new_supabase
                  .from('challenge')
                  .select('*')
                  .eq('isActive', 'true')
  ...
  const cashout1X1 = (data) => {
    ...
    new_supabase.from("challenge").select("*").eq("isActive", "true").then(response => { 
      ...
      new_supabase.from("challenge").update([{ data: newChallanged_data, isActive:false }]).eq("id", response.data[0].id).then(response2 => {
        new_supabase.from("user").update([{ wallet: newWallet }]).eq("id", state.loggedInUser.user.id).then(wallet_updated => {

          const meta = {code: response.data[0].code}
          new_supabase.from("feeds").insert([{type:"NEW CHALLANGE WIN", from:'ADMIN', to:"ALL", meta: meta}]).then(insert_response => { 
